...
res <- parLapply(cl, 1:nsim, function(sim) {
  runSimRICP(p = 5, k = 2, nenv = 200, renv = c(80, 100), rBeta = c(-5, 5), tau = 0.5,
...
for(metric in metrics) {
  df <- scores[[metric]][, -ncol(scores[[metric]]), drop = FALSE] %>%
    as.data.frame()
  methods <- c(rownames(df)[-nrow(df)], "R-ICP")
  df$method <- rownames(df) <- methods
  rowOrder <- c("random", "pooled regression", "GES", "LinGAM", "ICP", "nonlinearICP", "R-ICP")
